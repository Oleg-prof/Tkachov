...
//Записи для log_up.
$z='up_ang.php работает.';
$z1='Запись в файл Ang_pot.txt произведена успешно.';
$z2='Запись в файл Initial.txt произведена успешно.';
$z3='Запись в файл f_temp.txt произведена успешно';
$z4='Запись в файл table.txt произведена успешно';

//Записи ошибок.
$err_1='Ошибка открытия, записи файла Ang_pot.txt.';
...
//Переменные.
	...
	$k2=(float)$_POST["k2"];//Вторая производная.
	$k3=(float)$_POST["k3"];//Третья производная.
	$k4=(float)$_POST["k4"];//Четвертая производная.
	$wid=(float)$_POST["width"];//Ширина кривой.
	$nol=(float)$_POST["nol"];//Число уровней для расчета.
	$mass=(float)$_POST["mass"];//Масса частицы.
	
//Открытие файлов.
$pot=fopen("Ang_pot.txt","w");//Файл для постройки кривой.
...
if($pot)
	{fprintf($pot,"%d\n",$nop);//Число точек.
	fprintf($pot,"%f\n",$k2);//Вторая производная.
	fprintf($pot,"%f\n",$k3);//Третья производная.
	fprintf($pot,"%f\n",$k4);//Четвертая производная..
	fprintf($pot,"%f\n",$wid);//Ширина кривой.
	fprintf($log,"%s\n",$z1);}//Запись в log.
else
	{echo("Ошибка открытия файла Ang_pot.txt");
	fprintf($log,"%s\n",$err_1);//Запись в log.
