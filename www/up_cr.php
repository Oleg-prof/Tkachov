...
//Открытие файлов.
$pot=fopen("Cratzer.txt","w");//Файл для постройки кривой.
...
if($pot)
	...
	{echo("Ошибка открытия файла Cratzer.txt");
	fprintf($log,"%s\n",$err_1);//Запись в log.
...
if($fini)
	...
	fprintf($fini,"%f\n",$mass);//Масса частицы.
	fprintf($fini,"%f\n",$wid);//Ширина кривой.
	fprintf($fini,"%d\n",$nol);//Количество необходимых уровней.
	fprintf($log,"%s\n",$z2);}//Запись в log.
else
...
//Запуск программы расчета энергии по формулам.
